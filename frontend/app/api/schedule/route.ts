...
export async function GET(req: NextRequest) {
  const base = B();
  if (!base) return NextResponse.json({ channel: "", yt_times: "", ig_times: "", timezone: "", jobs: [] });
  try {
    const auth = req.headers.get("authorization") ?? "";
    const res  = await fetch(`${base}/api/schedule`, { headers: { Authorization: auth, ...NGROK } });
    const text = await res.text();
    try { return NextResponse.json(JSON.parse(text), { status: res.status }); }
    catch { return NextResponse.json({ channel: "", yt_times: "", ig_times: "", timezone: "", jobs: [] }); }
  } catch { return NextResponse.json({ channel: "", yt_times: "", ig_times: "", timezone: "", jobs: [] }); }
}
